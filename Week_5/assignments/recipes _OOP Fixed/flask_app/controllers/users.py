...
# this route displays all users with their recipes
@app.route("/user_with_recipe")
def get_user_with_recipe():
    # check if user is logged in
    if session.get("user_id")==None: 
        return redirect("/")
    else: 
        ...
        # in case no user came back
        if user.User.get_one_user_with_recipes(data) == None:
            return render_template ("users_with_recipe.html",user_to_display = user.User.get_one_user_by_id(data))
        else:
            return render_template ("users_with_recipe.html",user_to_display = user.User.get_one_user_by_id(data),user_with_recipes_to_display =  user.User.get_one_user_with_recipes(data))
